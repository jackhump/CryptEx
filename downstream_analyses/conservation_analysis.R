...
write.table(adjacent.exons.bed, paste0(outFolder,"/Feature_Enrichment/adjacent_exons.exons.bed"),sep ="\t", row.names= F, col.names =F, quote =F)
...
bed.files.list <- paste0(outFolder,"/Feature_Enrichment/",bed.names,".exons.bed")
...
if(grepl("Cleveland_TDP43", code)){
  ...
  graph_title <- paste0( gsub("_"," ", code) )
...
p <- ggplot(exon.conservation.merge, aes(x = gsub(".","\n",exon.type,fixed =T), y = phyloP.score, fill = exon.type, colour = exon.type)) + 
	geom_boxplot(colour = "black", outlier.shape = NA,notch = T ) +
	xlab("") + 
